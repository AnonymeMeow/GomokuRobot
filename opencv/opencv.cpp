...
namespace opencv
{
    ...
    double dis_sqr(const cv::Vec2f &a, const cv::Vec2f &b)
    {
        return (a - b).dot(a - b);
    }
    ...
    gomokuai::Cood_2D get_ai_step(int desired_count)
    {
        while (true)
        {
            ...
            // cv::imshow(window_title, grey);
            // cv::waitKey();
            std::vector<cv::Vec3f> circles;
            cv::HoughCircles(grey, circles, cv::HOUGH_GRADIENT, 1, 100, 50, 20, 50, 70);

            if (!(anchor_circle.size() > 0 && anchor_circles.size() >= 4))
            {
                continue;
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                dists.push_back({dis_sqr(main_anchor, anchor), anchor});
            }
            ...
            {
                cv::circle(img, cv::Point(circle[0], circle[1]), circle[2], cv::Scalar(0, 0, 0), 5, cv::LINE_AA, 0);
            }
            for (const auto& circle: white)
            {
                cv::circle(img, cv::Point(circle[0], circle[1]), circle[2], cv::Scalar(255, 255, 255), 5, cv::LINE_AA, 0);
            }
            cv::imshow(window_title, img);
            cv::waitKey(500);
